<?php
  ...
  $date3 = date('Y-m-d H:i:s', time());
   if($_FILES['logo']['name']!="" && isset($_POST['pid'])) {
      $errors= array();
      ...
      if(empty($errors)==true){
         ...
         $lgins=mysqli_query($con,"UPDATE `profile` SET `photo`='$filename2' WHERE `profile_id`='$profile_id' ");
   ...
   $upd1=mysqli_query($con,"UPDATE `profile` SET `name`='$name',`contact_1`='$ph1',`contact_2`='$ph2',`email`='$email',`address`='$address',`updated_at`='$date3' WHERE `profile_id`=$profile_id");
   $upd2=mysqli_query($con,"UPDATE `tbl_user` SET `name`='$name',`username`='$username',`password`='$password',`updated_at`='$date3'  WHERE `reference_id`=$profile_id");
   if($upd1 && $upd2 ){
