...
foreach($sheetData as $srow){
  ...
  if($qrw>0 && !empty($srow['B'])) {
    ...
    $ccheck=mysqli_query($con,"SELECT * FROM `warehouse` WHERE `warehouse_name` = '$whname' AND `org_id`='$oid' ");
    if(mysqli_num_rows($ccheck)<1){
      // print_r($srow);
      $gry=mysqli_query($con,"INSERT INTO `warehouse`(`warehouse_name`, `latitude`, `longitude`, `org_id`, `created_by`, `created_at`) VALUES ('$whname','$lat','$long','$oid','$uid','$date3')");
